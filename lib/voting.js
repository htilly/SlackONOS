  ...
  try {
    ...
    if (gongScore[userName] >= gongLimitPerUser) {
      ...
      if (gongCounter >= gongLimit) {
        ...
        clearVoteCountForTrack(current); // GONGed track leaves play, its vote-to-play tally is now orphaned
        gongCounter = 0;
        gongScore = {};
...
/**
 * Bulk-clear all vote-to-play state. Used when the entire queue disappears
 * (e.g. flush), where clearing per-track by key isn't possible/necessary.
 */
function clearAllTrackVotes() {
  trackVoteCount = {};
  trackVoteUsers = {};
  trackVoteMetadata = {};
  voteCounter = 0; // dead/unused field, reset for hygiene
  voteScore = {};
}

/**
 * Reset ALL voting-related state: gong, gong-immunity, vote-immune,
 * vote-to-play, and flush-vote. Backs the admin `resetvotes` command - a
 * manual safety valve for clearing orphaned state (e.g. leftover votes on
 * a track that's no longer in the queue).
 * @returns {Object} summary of what had active state before the reset
 */
function resetAllVoteState() {
  const summary = {
    gongVotesCleared: gongCounter > 0,
    immuneTracksCleared: Object.keys(gongBannedTracks).length,
    voteImmuneVotesCleared: voteImmuneCounter > 0,
    tracksWithVotesCleared: Object.keys(trackVoteCount).length,
    flushVotesCleared: flushVoteCounter > 0,
  };

  resetGongState();
  Object.keys(gongBannedTracks).forEach((k) => delete gongBannedTracks[k]); // const, mutate in place

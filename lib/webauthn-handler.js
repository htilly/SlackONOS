...
async function verifyRegistrationResponseHandler(req, body) {
  ...
  // Log the raw values to verify format
  logWebAuthn('info', 'Raw credential data from SimpleWebAuthn', {
    credentialIDType: typeof credentialID,
    credentialIDValue: typeof credentialID === 'string' ? credentialID.substring(0, 30) + '...' : 'NOT STRING',
    credentialIDIsBuffer: Buffer.isBuffer(credentialID),
    credentialIDIsUint8Array: credentialID instanceof Uint8Array,
    publicKeyType: typeof credentialPublicKey,
    publicKeyIsBuffer: Buffer.isBuffer(credentialPublicKey)
  });
  ...
  if (typeof credentialID === 'string') {
    ...
    logWebAuthn('info', 'Credential ID is already a base64url string, saving as-is (NO conversion)');
    ...
    logWebAuthn('info', 'Credential ID is Buffer (raw bytes), converted to base64url string');
    ...
    logWebAuthn('info', 'Credential ID is Uint8Array (raw bytes), converted to base64url string');
  } else {
  ...
  logWebAuthn('info', 'Saving credential with ID', {
    credentialID: newCredential.credentialID.substring(0, 30) + '...',
    credentialIDLength: newCredential.credentialID.length,
    deviceName: newCredential.deviceName
  });

  logWebAuthn('info', 'Saving new credential', {
    credentialID: newCredential.credentialID.slice(0, 16) + '...',
    deviceName: newCredential.deviceName,
  ...
  logWebAuthn('info', 'Credential saved successfully', {
    totalCredentials: existingCredentials.length,
    credentialID: newCredential.credentialID.slice(0, 16) + '...'
  });
...
async function generateAuthenticationOptionsHandler(req) {
  ...
  logWebAuthn('info', 'Generating authentication options', { rpId, origin });

  // Load credentials
  const credentialsData = await loadCredentials();
  const existingCredentials = credentialsData.credentials || [];

  logWebAuthn('info', 'Loaded credentials for authentication', { count: existingCredentials.length });
  ...
  try {
    ...
    const allowCredentials = existingCredentials.map(cred => {
      // Log to verify we're using the stored credential ID correctly
      logWebAuthn('info', 'Mapping credential for authentication', {
        storedCredentialID: cred.credentialID?.substring(0, 30) + '...',
        storedCredentialIDType: typeof cred.credentialID,
        storedCredentialIDLength: cred.credentialID?.length
      });
...
async function verifyAuthenticationResponseHandler(req, body) {
  logWebAuthn('info', 'verifyAuthenticationResponseHandler called', { 
  ...
  try {
    ...
    logWebAuthn('info', 'Request body parsed successfully', {
      hasId: !!data.id,
      hasRawId: !!data.rawId,
  ...
  try {
    ...
    logWebAuthn('info', 'Client data parsed', {
      challengeLength: receivedChallenge?.length,
  ...
  logWebAuthn('info', 'Searching for credential', {
  ...
  logWebAuthn('info', 'Authentication credential loaded', { 
    rpId, 
    origin, 
    credentialID: credentialID?.slice(0, 8) + '...', 
  ...
  logWebAuthn('info', 'Preparing credential for verification (v13 API)', {
    rpId,
    origin,
    requireUserVerification,
    storedCredentialID: credential.credentialID.substring(0, 30) + '...',
    storedCredentialIDLength: credential.credentialID.length,
    storedPublicKeyLength: credential.credentialPublicKey.length,
    counter: credentialCounter,
    hasTransports: !!(credential.transports && credential.transports.length > 0),
  ...
  try {
    ...
    logWebAuthn('info', 'Calling verifyAuthenticationResponse with credential (v13 API)', {
      hasResponse: !!data,
      hasExpectedChallenge: !!challenge,
      hasExpectedOrigin: !!origin,
      hasExpectedRPID: !!rpId,
      hasCredential: !!credentialObject,
      credentialKeys: Object.keys(credentialObject),
      credentialIdType: typeof credentialObject.id,
      credentialPublicKeyType: typeof credentialObject.publicKey,
      credentialIdLength: credentialObject.id.length,
      credentialPublicKeyLength: credentialObject.publicKey.length,
      credentialCounter: credentialObject.counter,
      credentialTransports: credentialObject.transports,
      // Debug response structure
      dataType: typeof data,
      dataResponseType: typeof data.response,
      dataResponseClientDataJSONType: typeof data.response?.clientDataJSON,
      dataResponseAuthenticatorDataType: typeof data.response?.authenticatorData,
      dataResponseSignatureType: typeof data.response?.signature,
    });
    ...
    logWebAuthn('info', 'Verification completed', {

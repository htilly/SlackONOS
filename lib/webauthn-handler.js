...
async function verifyRegistrationResponseHandler(req, body) {
  ...
  try {
    ...
    logWebAuthn('error', 'Registration verification failed', { error: err.message });
    challenges.delete(challenge);
...
async function verifyAuthenticationResponseHandler(req, body) {
  ...
  const data = JSON.parse(body);
  const { rpId, origin } = getRPConfig(req);
  logWebAuthn('info', 'Verifying authentication', { rpId, origin });
  const userId = getUserId();
  ...
  try {
    ...
    logWebAuthn('error', 'Authentication verification failed', { error: err.message });
    challenges.delete(challenge);
    throw new Error(`Verification failed: ${err.message}`);
  }

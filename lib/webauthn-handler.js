...
function getRPConfig(req) {
  ...
  let rpId = config.get('webauthnRpId') || 'localhost';
  let origin = config.get('webauthnOrigin');
  
  // Auto-detect origin from request if not set
  if (!origin && req) {
    const protocol = req.headers['x-forwarded-proto'] || 
                    (req.connection?.encrypted ? 'https' : 'http') ||
                    'https';
    const host = req.headers.host || `${rpId}:${config.get('webPort') || 8181}`;
    let hostname = host.split(':')[0];

    // WebAuthn does not allow raw IP addresses as rpId. Map loopback IPs to localhost for dev.
    if (hostname === '127.0.0.1' || hostname === '0.0.0.0') {
      hostname = 'localhost';
    }

    origin = `${protocol}://${hostname}${host.includes(':') ? ':' + host.split(':')[1] : ''}`;
    
    // Extract rpId from host if not explicitly set
    if (!config.get('webauthnRpId')) {
      rpId = hostname;
    }
  } else if (!origin) {
    origin = `https://${rpId}:${config.get('webPort') || 8181}`;
  }
...
async function verifyAuthenticationResponseHandler(req, body) {
  ...
  const credential = existingCredentials.find(
    cred => Buffer.from(cred.credentialID, 'base64url').toString('base64url') === credentialID
  );

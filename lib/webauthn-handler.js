...
function getRPConfig(req) {
  ...
  if (req) {
    // X-Forwarded-* headers are client-controlled and are only honored when
    // `trustProxy` is explicitly enabled - otherwise an attacker could steer
    // the rpId/origin WebAuthn binds credentials to, defeating its core
    // phishing-resistance property (see O-007). Mirrors the same gate
    // auth-handler.js's getClientIp() applies to X-Forwarded-For.
    const trustProxy = config.get('trustProxy') === true;
    ...
    // If an origin is explicitly configured (webauthnOrigin), honor it and do not
    // let request headers (Host/X-Forwarded-Host) override the phishing-binding origin.
  ...
  if (!origin) {
    const fallbackPort = config.get('webPort') || 8080;
    origin = `https://${rpId || 'localhost'}${fallbackPort && fallbackPort !== 443 ? ':' + fallbackPort : ''}`;
    if (!rpId) rpId = origin.split('://')[1].split(':')[0];
  }

  // Strip trailing slashes without an unbounded quantifier on user-influenced
  ...
  try {
    ...
    if (fileExists) {
      try {
        ...
        credentialCount = Array.isArray(parsed.credentials) ? parsed.credentials.length : 0;
      } catch (err) {
        // Ignore read/parse errors; never leak file contents to callers.
      }
    }

    // NOTE: /api/auth/webauthn/status is reachable WITHOUT authentication, so this
    // exposes only counts/metadata — never the credential records (public keys,
    // device names, timestamps) or the absolute file path.
    return {
      exists: fileExists,
      size: fileSize,
      credentialCount

...
async function generateRegistrationOptionsHandler(req) {
  ...
  // Configure authenticator selection to support both platform (Touch ID) and cross-platform (Yubikey) authenticators
  // According to WebAuthn L3 spec and SimpleWebAuthn v13 best practices:
  // - Don't set authenticatorAttachment to allow both types
  // - Use 'discouraged' for userVerification by default to maximize compatibility with all authenticator types
  // - Set residentKey to 'preferred' (not 'required') to support both discoverable and non-discoverable credentials
  const authenticatorSelection = {
    ...
    // No authenticatorAttachment = allow both platform (Touch ID) and cross-platform (Yubikey)
  };

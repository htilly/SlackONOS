...
function createWebServer({
  ...
  function serveSetupStaticFile(res, urlPath) {
    ...
    // Guard clause in the exact form CodeQL's js/path-injection sanitizer
    // recognizes: an early return gated directly on a single
    // `.startsWith(root + sep)` call (a prior version that stored an
    // equivalent boolean in an intermediate variable was not recognized
    // as a barrier). normalizedPublicDir itself can never satisfy this,
    // so requesting the setup dir with no filename is rejected too.
    if (!normalizedFilePath.startsWith(normalizedPublicDir + path.sep)) {

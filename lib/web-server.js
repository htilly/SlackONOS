...
function createWebServer({
  ...
  function serveSetupStaticFile(res, urlPath) {
    ...
    // Containment check in the startsWith(root + sep) form CodeQL's
    // js/path-injection sanitizer recognizes (the equivalent
    // path.relative(...).startsWith('..') form above was not being
    // recognized as a barrier).
    const isWithinPublicDir = normalizedFilePath !== normalizedPublicDir &&

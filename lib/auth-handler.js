...
/**
 * Whether proxy-supplied forwarding headers should be trusted.
 * Only enable this when the app sits behind a reverse proxy you control.
 */
function isProxyTrusted() {
  try {
    const config = nconf.file({ file: CONFIG_PATH });
    return config.get('trustProxy') === true;
  } catch {
    return false;
  }
}
 ...
 *
 * X-Forwarded-For / X-Real-IP are client-controlled and are only honored when
 * `trustProxy` is explicitly enabled. Otherwise an attacker could send a unique
 * forwarded IP per request and completely bypass login rate limiting; we fall
 * back to the real socket peer address instead.
 */

...
function flush(input, channel, userName) {
  ...
  sonos
    ...
    .then(() => {
      ...
      if (voting && typeof voting.clearAllTrackVotes === 'function') {
...
function nextTrack(channel, userName) {
  ...
  sonos
    .currentTrack()
    .catch(() => null) // don't block the skip if this lookup fails
    .then((outgoingTrack) => sonos.next().then(() => outgoingTrack))
    .then((outgoingTrack) => {
      if (outgoingTrack && voting && typeof voting.clearVoteCountForTrack === 'function') {
        voting.clearVoteCountForTrack(outgoingTrack);
      }
...
function removeTrack(input, channel) {
  ...
  const zeroBasedIndex = parseInt(input[1]);
  if (isNaN(zeroBasedIndex)) {
    sendMessage('🤔 That\'s not a valid track number. Check the queue with `list`! 📋', channel);
    return;
  }
  const trackNb = zeroBasedIndex + 1; // +1 because Sonos uses 1-based indexing

  // Look up the track's identity before removing it, so any orphaned
  // vote-to-play tally for it can be cleared too.
  sonos
    .getQueue()
    .then((result) => {
      const item = result && Array.isArray(result.items) ? result.items[zeroBasedIndex] : null;
      return sonos.removeTracksFromQueue(trackNb, 1).then(() => item);
    })
    .then((item) => {
      queueCache.adjustTotal(-1, 'removeTrack');
      logger.info('Removed track with index: ' + trackNb);
      if (item && voting && typeof voting.clearVoteCountForTrack === 'function') {
        voting.clearVoteCountForTrack(item);
      }
...
function purgeHalfQueue(input, channel) {
  sonos
    ...
    .then((result) => {
      ...
      const removedItems = Array.isArray(result.items)
        ? result.items.slice(halfQueue - 1, halfQueue - 1 + halfQueue)
        : [];
      sonos
        .removeTracksFromQueue(halfQueue, halfQueue)
        .then(() => {
          queueCache.adjustTotal(-halfQueue, 'purgeHalfQueue');
          if (voting && typeof voting.clearVoteCountForTrack === 'function') {
            removedItems.forEach((item) => voting.clearVoteCountForTrack(item));
          }
...
/**
 * Reset all voting-related state (gong, gong-immunity, vote-immune,
 * vote-to-play, flush-vote). Manual safety valve for clearing orphaned
 * state, e.g. leftover votes on a track that's no longer in the queue.
 */
function resetVotes(input, channel, userName) {
  logUserAction(userName, 'resetvotes');
  if (!voting || typeof voting.resetAllVoteState !== 'function') {
    sendMessage('⚠️ Voting system unavailable, nothing to reset.', channel);
    return;
  }
  const summary = voting.resetAllVoteState();
  sendMessage(
    '🧹 *VOTE STATE RESET!* All democracy has been wiped clean:\n' +
    `> 🔔 Gong votes: cleared${summary.gongVotesCleared ? ' (was active)' : ''}\n` +
    `> 🛡️ Immune tracks: ${summary.immuneTracksCleared} cleared\n` +
    `> 🗳️ Vote-immune votes: cleared${summary.voteImmuneVotesCleared ? ' (was active)' : ''}\n` +
    `> ⭐ Track votes: ${summary.tracksWithVotesCleared} track(s) cleared\n` +
    `> 🚽 Flush votes: cleared${summary.flushVotesCleared ? ' (was active)' : ''}\n` +
    'Fresh start! 🎉',
    channel
  );
}

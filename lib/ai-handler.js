...
const { isUnsafeObjectKey } = require('./safe-object-key');
...
function buildContextKey(userName, scope = null) {
  ...
  if (!scope || typeof scope !== 'object') {
    // Security (defense-in-depth): every current caller passes a scope
    // object, so this bare branch isn't reachable with attacker-controlled
    // input today. But this key is used as a plain-object key in
    // `userContext[contextKey] = {...}` (setUserContext), so if a future
    // caller ever omits scope, a userName of '__proto__' would otherwise
    // pollute Object.prototype process-wide. Prefix so it can never equal
    // that (or 'constructor'/'prototype') exactly.
    return isUnsafeObjectKey(normalizedUser) ? `user:${normalizedUser}` : normalizedUser;
  }

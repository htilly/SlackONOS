...
function noteQueuedTracks(count, source) {
  const normalizedCount = Number(count);
  if (!Number.isFinite(normalizedCount) || normalizedCount <= 0) {
    return;
  }

  queueCache.adjustTotal(normalizedCount, source);
}
...
async function add(input, channel, userName) {
  ...
  logUserAction(userName, 'add', { query: track, type: 'track' });
  ...
  try {
    ...
    if (queue) {
      queueCache.updateFromQueue(queue, 'add:prefetch');
    }

    // Check for duplicates before handling stopped state. Some Sonos devices can
    // briefly report stopped while the queue still contains the just-added track.
    ...
    if (state === 'stopped') {
      ...
      try {
        ...
        await sonos.flush();
        queueCache.setTotal(0, 'add:flush');
        await new Promise(resolve => setTimeout(resolve, 300));
    ...
    try {
      ...
      if (state === 'stopped') {
        queueCache.setTotal(1, 'add');
      } else {
        noteQueuedTracks(1, 'add');
      }
    } catch (e) {
...
async function addalbum(input, channel, userName) {
  ...
  logUserAction(userName, 'addalbum', { query: album, type: 'album' });
  ...
  try {
    ...
    if (isStopped) {
      ...
      try {
        ...
        await sonos.flush();
        queueCache.setTotal(0, 'addalbum:flush');
        await new Promise(resolve => setTimeout(resolve, 300));
        logger.info('Queue flushed and ready');
      } catch (flushErr) {
        logger.warn('Could not flush queue: ' + flushErr.message);
    ...
    (async () => {
      try {
        if (blacklistedTracks.length > 0) {
          ...
          logger.info(`Added ${queuedCount} tracks from album (filtered ${blacklistedTracks.length})`);
          if (queuedCount > 0) {
            noteQueuedTracks(queuedCount, 'addalbum');
          }
        } else {
          const queueResult = await sonos.queue(result.uri);
          result.queuePosition = getFirstQueuedTrackNumber(queueResult, 1);
          logger.info('Added album: ' + result.name);
          noteQueuedTracks(albumTracks.length, 'addalbum');
        }
...
async function addplaylist(input, channel, userName) {
  ...
  logUserAction(userName, 'addplaylist', { query: playlist, type: 'playlist' });
  ...
  try {
    ...
    if (isStopped) {
      ...
      try {
        ...
        await sonos.flush();
        queueCache.setTotal(0, 'addplaylist:flush');
        await new Promise(resolve => setTimeout(resolve, 300));
        logger.info('Queue flushed and ready');
      } catch (flushErr) {
        logger.warn('Could not flush queue: ' + flushErr.message);
    ...
    if (blacklistedTracks.length > 0) {
      ...
      noteQueuedTracks(allowedTracks.length, 'addplaylist');
    } else {
      const queueResult = await sonos.queue(result.uri);
      queuedTrackNumber = getFirstQueuedTrackNumber(queueResult, 1);
      logger.info('Added playlist: ' + result.name);
      noteQueuedTracks(playlistTracks.length, 'addplaylist');
    }
...
async function append(input, channel, userName) {
  ...
  const track = input.slice(1).join(' ');
  logUserAction(userName, 'append', { query: track, type: 'track' });
  ...
  try {
    ...
    try {
      ...
      queueCache.updateFromQueue(queue, 'append:prefetch');

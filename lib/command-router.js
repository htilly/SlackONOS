...
function cleanCommandText(text) {
  text = (text || '').trim();
  text = text.replace(/^(&gt;|>)\s*/, '');
  // Decode all HTML entities in a single pass so a chained &amp;lt; etc.
  // can't be left partially decoded (CodeQL js/double-escaping).
  const HTML_ENTITIES = { '&gt;': '>', '&lt;': '<', '&amp;': '&', '&quot;': '"' };
  text = text.replace(/&(?:gt|lt|amp|quot);/g, match => HTML_ENTITIES[match]);
...
function createCommandRouter(deps) {
  ...
  function isUnauthorizedAdminCommand(command, channel, platform, isAdmin) {
    const resolved = getCommandMeta(command);
    if (!resolved || !resolved.cmdMeta || !resolved.cmdMeta.admin) {
      return false;
    }

    return !isAuthorizedCommand(command, channel, platform, isAdmin);
  }

  function isExplicitAdminIntent(command, cleanText) {
    const text = String(cleanText || '').trim().toLowerCase();
    if (!text) {
      return false;
    }

    const exactOrCommandPrefix = new RegExp(`^${command.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`);

    switch (command) {
      case 'flush':
        return exactOrCommandPrefix.test(text) ||
          /\b(clear|empty|flush|wipe|reset|rensa|töm|tom)\b.*\b(queue|list|kön|kon|listan)\b/.test(text);
      case 'next':
        return /^(next|skip|skippa|nästa)(\s+(track|song|låt|lat|this|current|one|it|please|pls|now|nu|den|låten|laten))?$/.test(text) ||
          /\b(skip|skippa)\b.*\b(track|song|låt|lat|this|current|one|it|den|låten|laten)\b/.test(text);
      case 'play':
        return /^play(\s+(it|music|sonos|queue|now|nu))?$/.test(text) ||
          /^play\s+(track\s*)?\d+\s*$/.test(text) ||
          /\b(resume|start|continue)\b.*\b(playback|music|sonos|queue)\b/.test(text) ||
          /\b(återuppta|ateruppta|fortsätt|fortsatt)\b/.test(text);
      case 'pause':
        return /^pause(\s+(it|this|current|one|music|playback|sonos|queue|track|song|låt|lat|den|låten|laten))?$/.test(text) ||
          /\b(pause|pausa)\b.*\b(playback|music|sonos|queue|track|song|låt|lat)\b/.test(text);
      case 'resume':
        return /^(resume|playpause)(\s+(it|music|playback|sonos|queue|now|nu))?$/.test(text) ||
          /\b(resume|unpause|continue)\b.*\b(playback|music|sonos|queue)\b/.test(text) ||
          /\b(återuppta|ateruppta|fortsätt|fortsatt)\b/.test(text);
      case 'stop':
        return /^stop(\s+(it|this|current|one|music|playback|sonos|queue|track|song|låt|lat|den|låten|laten))?$/.test(text) ||
          /\b(stop|stopp|stoppa)\b.*\b(playback|music|sonos|queue|track|song|låt|lat)\b/.test(text);
      case 'previous':
        return /^(previous|prev|back|föregående|foregaende)(\s+(track|song|låt|lat))?$/.test(text);
      case 'shuffle':
        return /^shuffle\s*$/.test(text) ||
          /\b(enable|turn on|start|activate)\b.*\bshuffle\b/.test(text) ||
          /\b(slumpa|blanda)\b.*\b(queue|list|kön|kon|listan)\b/.test(text);
      case 'normal':
        return /^normal\s*$/.test(text) ||
          /\b(disable|turn off|stop)\b.*\bshuffle\b/.test(text) ||
          /\b(normal)\b.*\b(play|mode|läge|lage)\b/.test(text);
      case 'setvolume':
        return /^(setvolume|set volume|volume to|turn volume)\b/.test(text);
      case 'setcrossfade':
        return /^(setcrossfade|crossfade|set crossfade|turn crossfade)\b/.test(text);
      case 'setconfig':
        return /^(setconfig|set config|config|getconfig)\b/.test(text) ||
          /^set\s+[a-z0-9_.-]+\s+(to\s+)?/.test(text);
      case 'remove':
        return /^(remove|delete)\s+(\d+|track|song|låt|lat)\b/.test(text);
      case 'move':
        return /^(move|mv)\s+(\d+|track|song|låt|lat)\b/.test(text);
      case 'blacklist':
      case 'trackblacklist':
      case 'debug':
      case 'telemetry':
  ...
  function stripMusicRequestVerb(cleanText) {
    return String(cleanText || '')
      .trim()
      .replace(/^(add|play|spela|lägg till|lagg till)\s+/i, '')
      .trim();
  }

  function looksLikeMusicFallback(cleanText) {
    ...
    if (/^(what|who|when|where|why|how|vad|vem|när|nar|var|hur)\s+(is|are|do|does|did|can|could|would|should|will|was|were|är|ar|gör|gor|kan|ska|skulle)\b/.test(lower)) {
      return false;
    }

    if (/^(add|play|spela|lägg till|lagg till)\s+\S+/.test(lower)) {
      return true;
    }
  ...
  function coerceAmbiguousUnauthorizedAIAdminCommand(parsed, cleanText, channel, platform, isAdmin) {
    if (!parsed || !isUnauthorizedAdminCommand(parsed.command, channel, platform, isAdmin)) {
      return parsed;
    }
    ...
    if (isExplicitAdminIntent(parsed.command, cleanText) || !looksLikeMusicFallback(cleanText)) {
      return parsed;
    }

    const query = stripMusicRequestVerb(cleanText);
    if (!query) {
      return parsed;
    }

    logger.warn(`AI parsed "${cleanText}" as unauthorized admin command "${parsed.command}", treating it as music add fallback`);
    return {
      ...parsed,
      command: 'add',
      args: [query],
      targetType: parsed.targetType === 'command' ? 'track' : (parsed.targetType || 'track'),
      reasoning: `${parsed.reasoning || 'AI command parse'}; non-admin ambiguous control parse treated as a music request.`,
      summary: parsed.summary || `Track radar locked on ${query}.`,
      followUp: null
    };
  }
  ...
  async function handleNaturalLanguage(text, channel, userName, platform = 'slack', isAdmin = false) {
    ...
    let disambiguatingAdminPhrase = false;
    ...
    if (isKnownCommand(firstWord) && !looksLikeNaturalLang) {
      ...
      if (cmdMeta && cmdMeta.admin && !isExplicitAdminIntent(cmdKey, cleanText)) {
        disambiguatingAdminPhrase = true;
        logger.info(`>>> Proceeding to AI for ambiguous admin-looking phrase "${cleanText}"`);
      } else {
    ...
    if (isKnownCommand(firstWord) && !disambiguatingAdminPhrase) {
    ...
    try {
      ...
      parsed = coerceAmbiguousUnauthorizedAIAdminCommand(parsed, cleanText, channel, platform, isAdmin);
